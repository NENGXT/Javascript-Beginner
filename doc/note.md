...
-   `.some()` : 检查数组中是否有满足的**条件**的元素，满足的话返回 true，不满足的话返回 flase
    -   代码事例：

```javascript
const arr = [200, 300, -100, -500];
const testDate = arr.some((mov) => mov < 0);
console.log(testDate); //返回true，因为-100和-500都小于0
```

-   `.every()` : 检查数组是否时**所有的元素**满足条件，满足的话返回 true，不满足的话返回 flase
    -   代码事例：

```javascript
const arr = [200, 300, -100, -500];
console.log(arr.every((mov) => mov > 0)); //返回false，因为-100和-500都小于0，所以数组中并不是所有元素都大于0
```

### 分离回调函数（Extracting Callback Functions）

**什么是分离回调函数？**

在使用数组方法（如 `.map()`、`.filter()`、`.some()`、`.forEach()` 等）时，常见的写法是直接在括号里写一个匿名函数。但如果逻辑较复杂或需要复用，就可以**将这个回调函数单独定义出来**，再传进去使用。

```javascript
const arr = [1, 2, 3];
const clac = (acc, cur) => acc + cur;
console.log(arr.reduce(clac)); //输出6
```

### flat and flatMap

-   `.flat()`:用于“拍平”嵌套的数组结构，也就是把里面嵌套的子数组展开成一个更扁平的数组。
    -   语法结构： `.flat(处理的嵌套数)`
    -   代码示例：
...
console.log(arr.flat(2)); //输出[1,...,7,8]
```

-   `.flatMap()`:同样用于“拍平”嵌套的数组结构，并且拥有`.map()`的功能

### 数组排序 Sorting

-   `.sort()`:排序方法，默认按照字符串的方式排序，排序后的数组会改变**原数组**
-   基本语法：
